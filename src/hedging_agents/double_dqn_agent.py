"""
Double DQN with dual Q-functions for risk-aware hedging.

Same as DQN but with reduced overestimation bias:
  - ONLINE networks select the greedy action  a* = argmin_a F_online(s', a)
  - TARGET networks evaluate it:  Q1_target(s', a*),  Q2_target(s', a*)

This is the discrete-action counterpart of Cao et al. (2021) Section 3.4.
"""
...
class DoubleQDNHedgingAgent(AbstractHedgingAgent):
    """
    Double DQN with dual Q-functions (Q1, Q2) to minimise
    Y = E[C] + λ·σ(C)  as in Cao et al. (2021).

    Difference vs DQN: online nets select the next greedy action,
    target nets evaluate it (reduces overestimation bias).
    """
    ...
    def _greedy_action_idx(self, state_t: torch.Tensor) -> int:
        ...
        return int(torch.argmin(self._F_values(q1, q2), dim=1).item())
    ...
    def learn(self) -> float | None:
        ...
        # Current estimates
        q1_sa = self.q1_net(batch.states).gather(1, action_indices)
        q2_sa = self.q2_net(batch.states).gather(1, action_indices)

        with torch.no_grad():
            not_done = 1.0 - batch.dones

            # ── Double DQN: ONLINE selects, TARGET evaluates ────────
            next_q1_online = self.q1_net(batch.next_states)
            next_q2_online = self.q2_net(batch.next_states)
            next_f = self._F_values(next_q1_online, next_q2_online)
            next_greedy = next_f.argmin(dim=1, keepdim=True)

            next_q1_val = self.q1_target(batch.next_states).gather(1, next_greedy)
            next_q2_val = self.q2_target(batch.next_states).gather(1, next_greedy)

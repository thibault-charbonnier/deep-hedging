"""
DQN with dual Q-functions + Prioritized Experience Replay.
Discrete-action variant of Cao et al. (2021) Section 3.4.
"""
...
class DQNHedgingAgent(AbstractHedgingAgent):
    ...
    def act(self, state, eval_mode=False):
        ...
        with torch.no_grad():
            ...
            f = self._F_values(self.q1_net(st), self.q2_net(st))
            return self._index_to_action(int(torch.argmin(f, dim=1).item()))
    ...
    def learn(self):
        ...
        with torch.no_grad():
            ...
            nq1t = self.q1_target(batch.next_states)
            nq2t = self.q2_target(batch.next_states)
            nf = self._F_values(nq1t, nq2t)
            ng = nf.argmin(dim=1, keepdim=True)
            nq1v = nq1t.gather(1, ng)
            nq2v = nq2t.gather(1, ng)
